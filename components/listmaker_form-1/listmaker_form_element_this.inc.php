...
/*
	...
	['name:name@{ @ -> title in element }','forced place holder { not-array not-inDiv not-text:..  }'],
...
function listmaker_form_element_this( $list ){
	...
	# 
	# array of elements
	$aol = [ 'text','color','name','number','textarea','select','checkbox','radio','date','clock','toggle','url','email','file','submit','hidden','password','cat','position' ];
...
function lmfe_this_bake( $info ){
	...
	} else if( in_array( $info['type'] , ['file','submit'] ) ){
		;//
	...
	return lmfe_this_bake_this( $info );
}


function lmfe_this_bake_this( $info ){
